'''Categorize word lists by part of speech.

Many word lists you find are not divided by parts of speech and are just broken into categories.
But when generating random names, we'd preferrably like to impose gramatical structure on the 
words we generate.

This will divide them into folders broken out by part of speech. If a word can function as multiple different
POS, then it will be added to all applicable ones.
'''
import os
import glob
import re
